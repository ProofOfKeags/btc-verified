...
/-!
  # Private native transaction boundary

  This module is the pure Lean half of the private ABI used by the
  ...
  impose a raw total-input-size cap or change the protocol codec.
  ...
  The generated C wrappers consume every object argument and return owned object
  results. A native caller borrowing an object must increment it before calling
  an export.
...
The transaction object is owned at the generated C ABI. -/
...
/-- Decide Core's standalone context-free transaction predicate: the regular
checker plus its single-null-input coinbase branch. The transaction object is
owned at the generated C ABI. Reference: pinned Core
...
/-- Return the transaction locktime. The transaction object is owned at the
generated C ABI. -/
@[export btcv_kernel_locktime]
def locktime (tx : Tx) : UInt32 :=
  tx.body.lockTime

/-- Materialize the transaction inputs in wire order. The transaction object is
owned at the generated C ABI, and the returned array is owned. -/
@[export btcv_kernel_inputs]
def inputs (tx : Tx) : Array TxIn :=
  tx.body.inputs.val.toArray

/-- Materialize the transaction outputs in wire order. The transaction object
is owned at the generated C ABI, and the returned array is owned. -/
...
transaction inputs. Legacy inputs receive empty witness arrays. The transaction
object is owned at the generated C ABI, and all returned arrays are owned. -/
...
/-- Return an input's previous transaction id as its 32 raw wire bytes. The
input object is owned at the generated C ABI, and the byte array is owned. -/
@[export btcv_kernel_input_txid]
def inputTxid (input : TxIn) : ByteArray :=
  input.prevout.txid.val.toByteArray

/-- Return an input's previous-output index. The input object is owned at the
generated C ABI. -/
@[export btcv_kernel_input_index]
def inputIndex (input : TxIn) : UInt32 :=
  input.prevout.vout

/-- Return an input's sequence number. The input object is owned at the
generated C ABI. -/
@[export btcv_kernel_input_sequence]
def inputSequence (input : TxIn) : UInt32 :=
  input.sequence

/-- Return an input's raw scriptSig bytes. The input object is owned at the
generated C ABI, and the byte array is owned. -/
@[export btcv_kernel_input_script]
def inputScript (input : TxIn) : ByteArray :=
  input.scriptSig.code.val.toByteArray

/-- Return all 64 output-amount wire bits. The output object is owned at the
generated C ABI. -/
@[export btcv_kernel_output_amount]
def outputAmount (output : TxOut) : UInt64 :=
  output.value

/-- Return an output's raw scriptPubKey bytes. The output object is owned at the
generated C ABI, and the byte array is owned. -/
