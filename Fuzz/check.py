...
ASAN_OPTIONS_ENV = "ASAN_OPTIONS"
LSAN_OPTIONS_ENV = "LSAN_OPTIONS"
CONFORMANCE_ASAN_OPTIONS = "detect_leaks=0"
...
class EvidenceRun:
    ...
    def phase(self, name: str, command: list[str | Path], *,
        ...
        # Sanitizer runtime policy is selected explicitly per harness phase.
        child_environment.pop(ASAN_OPTIONS_ENV, None)
        child_environment.pop(LSAN_OPTIONS_ENV, None)
...
def main() -> int:
    ...
    try:
        ...
        execution_environment = (
            {ASAN_OPTIONS_ENV: CONFORMANCE_ASAN_OPTIONS}
            if "address" in sanitizers.split(",") else {})
        ...
        run.report["instrumentation"] = {
            ...
            "address_sanitizer_scope": (
                "Core and the C++ harness; prebuilt Lean objects and runtime are not rebuilt"
                if "address" in sanitizers.split(",") else "unavailable"),
            "leak_detection": {
                "status": "disabled",
                "scope": "transaction-conformance",
                "reason": "memory-lifetime assurance is outside this semantic comparison",
                "runtime_option": execution_environment.get(ASAN_OPTIONS_ENV),
            },
        }
        run.write()
        ...
        reproduce_script.write_text(
            ...
            f"unset {FAILURE_ENV}\n"
            f"unset {ASAN_OPTIONS_ENV}\n"
            f"unset {LSAN_OPTIONS_ENV}\n"
