...
-- Scan-BEFORE-pad (CVE-2012-2459): in `[1, 2, 2]` the duplicate is the padded
-- odd element, so the scan never compares it to its twin and `mutated` is
-- false; scanning after padding would wrongly flag `(2, 2)`. Also canonical.
#guard !(Impl.BitcoinCore.computeMerkleRoot ([1, 2, 2] : List Hash256)).2
#guard Merkle.canonicalCheck ([1, 2, 2] : List Hash256)
-- A duplicated run trips it one level up, where the pair aligns.
#guard (Impl.BitcoinCore.computeMerkleRoot ([1, 2, 3, 4, 5, 6, 5, 6] : List Hash256)).2
...
#guard (Impl.BitcoinCore.computeMerkleRoot ([7, 7] : List Hash256)).2
#guard (Impl.BitcoinCore.computeMerkleRoot ([1, 1, 2, 3] : List Hash256)).2
