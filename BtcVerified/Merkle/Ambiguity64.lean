...
theorem encode_length_of_isMinimal64 {b : TxBody} (h : IsMinimal64Body b) :
  ...
  rw [encode_txin_length hss, encode_txout_length hspk,
    CompactSize.encode_length_ofNat_lt_253 (show 1 < 253 by decide)]
  omega
...
theorem isMinimal64_of_encode_length {b : TxBody}
  ...
  have hcin := CompactSize.encode_length_ge_one (UInt64.ofNat b.inputs.val.length)
  have hcout := CompactSize.encode_length_ge_one (UInt64.ofNat b.outputs.val.length)
  ...
  rw [CompactSize.encode_length_ofNat_lt_253 (show 1 < 253 by decide), encode_txin_length_eq i,
    encode_txout_length_eq o, encode_script_length_eq i.scriptSig,
    encode_script_length_eq o.scriptPubKey] at hbody
  have hcsSS := CompactSize.encode_length_ge_one (UInt64.ofNat i.scriptSig.code.val.length)
  have hcsSPK := CompactSize.encode_length_ge_one (UInt64.ofNat o.scriptPubKey.code.val.length)
  rw [CompactSize.encode_length_ofNat_lt_253 (show i.scriptSig.code.val.length < 253 by omega),
    CompactSize.encode_length_ofNat_lt_253 (show o.scriptPubKey.code.val.length < 253 by omega)] at hbody
  omega
...
theorem forbiddenPreimageBytes_of_isForbidden {P : List UInt8}
    ...
    rw [CompactSize.encode_ofNat_singleton (show (1 : Nat) < 253 by decide),
      CompactSize.encode_ofNat_singleton (show ss.length < 253 by omega),
      CompactSize.encode_ofNat_singleton (show spk.length < 253 by omega)] at hP
    ...
    · -- input-count byte at position 4
      ...
      rw [hg, List.getElem?_append_cons (k := 4) (by rw [lev])]
      decide
    · -- scriptSig-length byte at position 41
      have hg : P = (Codec.encode v ++ (UInt64.ofNat 1).toUInt8 :: Codec.encode op)
          ++ (UInt64.ofNat ss.length).toUInt8 :: (ss ++ Codec.encode seq
            ...
            ++ (UInt64.ofNat spk.length).toUInt8 :: (spk ++ Codec.encode lt))) := by
        rw [hP]; simp only [List.append_assoc, List.cons_append, List.nil_append]
      rw [hg, List.getElem?_append_cons (k := 41)
    ...
    · -- output-count byte at position 46 + x
      have hg : P = (Codec.encode v ++ (UInt64.ofNat 1).toUInt8 ::
          ...
          ++ (UInt64.ofNat 1).toUInt8 :: (Codec.encode val
            ++ (UInt64.ofNat spk.length).toUInt8 :: (spk ++ Codec.encode lt)) := by
        rw [hP]; simp only [List.append_assoc, List.cons_append, List.nil_append]
      rw [hg, List.getElem?_append_cons (k := 46 + ss.length)
        (by simp only [List.length_append, List.length_cons]; omega)]
      decide
    · -- scriptPubKey-length byte at position 55 + x
      have hg : P = (Codec.encode v ++ (UInt64.ofNat 1).toUInt8 ::
          ...
          ++ (UInt64.ofNat spk.length).toUInt8 :: (spk ++ Codec.encode lt) := by
        rw [hP]; simp only [List.append_assoc, List.cons_append, List.nil_append]
      rw [hg, List.getElem?_append_cons (k := 55 + ss.length)
        (by simp only [List.length_append, List.length_cons]; omega),
        Option.map_some, toNat_compactSizeByte (by omega)]
      congr 1
      omega
...
theorem isForbidden_of_forbiddenPreimageBytes {P : List UInt8}
  ...
  have c4 : CompactSize.encode (UInt64.ofNat 1) = (P.drop 4).take 1 := by
    rw [List.drop_take_one (show 4 < P.length by omega), CompactSize.encode_ofNat_singleton (by decide),
      markerByte_eq (show 4 < P.length by omega) h4 (by decide)]
  have c41 : CompactSize.encode (UInt64.ofNat x) = (P.drop 41).take 1 := by
    rw [List.drop_take_one (show 41 < P.length by omega), CompactSize.encode_ofNat_singleton (by omega),
      markerByte_eq (show 41 < P.length by omega) h41 (by omega)]
  have c46 : CompactSize.encode (UInt64.ofNat 1) = (P.drop (46 + x)).take 1 := by
    rw [List.drop_take_one (show 46 + x < P.length by omega), CompactSize.encode_ofNat_singleton (by decide),
  ...
  have c55 : CompactSize.encode (UInt64.ofNat (4 - x)) = (P.drop (55 + x)).take 1 := by
    rw [List.drop_take_one (show 55 + x < P.length by omega), CompactSize.encode_ofNat_singleton (by omega),
  ...
  nth_rewrite 1 [c4]
  rw [List.take_merge (show 4 + 1 = 5 from rfl), List.take_merge (show 5 + 36 = 41 from rfl), c41,
    List.take_merge (show 41 + 1 = 42 from rfl), List.take_merge (show 42 + x = 42 + x from rfl),
    List.take_merge (show (42 + x) + 4 = 46 + x by omega)]
  nth_rewrite 1 [c46]
  rw [List.take_merge (show (46 + x) + 1 = 47 + x by omega),
    List.take_merge (show (47 + x) + 8 = 55 + x by omega), c55,
    List.take_merge (show (55 + x) + 1 = 56 + x by omega),
    List.take_merge (show (56 + x) + (4 - x) = 60 by omega), List.take_append_drop]
...
theorem layout_byte41 {P : List UInt8} {v : UInt32} {op : OutPoint} {ss : List UInt8}
  ...
  rw [CompactSize.encode_ofNat_singleton (show (1 : Nat) < 253 by decide),
    CompactSize.encode_ofNat_singleton hsslt] at hP
  have hg : P = (Codec.encode v ++ (UInt64.ofNat 1).toUInt8 :: Codec.encode op)
      ++ (UInt64.ofNat ss.length).toUInt8 :: (ss ++ Codec.encode seq
        ++ (UInt64.ofNat 1).toUInt8 :: (Codec.encode val
...
theorem valueField_decode {P : List UInt8} (v : UInt32) (op : OutPoint) (ss : List UInt8)
  ...
  have hpref : (Codec.encode v ++ CompactSize.encode (UInt64.ofNat 1) ++ Codec.encode op
    ...
    simp only [List.length_append, encode_uint32_length, encode_outpoint_length,
      CompactSize.encode_length_ofNat_lt_253 (show (1 : Nat) < 253 by decide),
      CompactSize.encode_length_ofNat_lt_253 hsslt]
    omega
