...
- **Naming**: rigid Lean/mathlib casing. `UpperCamelCase` for types, props,
  ...
  `CONTRIBUTING.md`.
- **Instance names**: every handwritten public `instance` has an explicit
  name, following `inst<Class><TargetOrPredicate>` (e.g. `instCodecTxOut`,
  `instDecidableMerkleCommits`), with a meaningful qualifier when needed.
  This gives audits, allowlists, proofs, and documentation stable declaration
  names. Instances generated by `deriving` keep their generated names;
  private and local instances may be anonymous. Audit the class and target
  from the instance's type, never by interpreting its name.
